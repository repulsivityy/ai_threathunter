...
class URLScanTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        ...
        try:
            ...
            # Make the API request with debug logging
            response_data = self._make_urlscan_request(search_url, clean_query)
            
            if response_data is None:
                return f'URLScan search failed for query "{query}"'
            
            # Format results - let AI analyze patterns
            return self._format_urlscan_results(response_data, clean_query)
            
        except Exception as error:
            print(f'URLScan search error: {error}')
            return f'URLScan search failed for query "{query}": {str(error)}'

    def _make_urlscan_request(self, url: str, query: str) -> Dict[str, Any]:
        """Make URLScan API request with debug logging."""
        ...
        request_info = {
            ...
            'method': 'GET',
            'query': query,
            'headers': {k: v if k != 'API-Key' else 'REDACTED' for k, v in headers.items()},
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                if debug_manager.debug_enabled:
                    debug_manager.log_api_call(
                        ...
                        response_data={'status_code': 429, 'message': 'Rate limited'},
                        error=None,
            ...
            if not response.ok:
                error_msg = f"URLScan API error: {response.status_code} {response.reason} - {response.text}"
            ...
            if debug_manager.debug_enabled:
                # Create a summary of the response to avoid logging massive amounts of data
                response_summary = {
                ...
                if response_data.get('results'):
                    response_summary['sample_results'] = response_data['results'][:3]
            ...
            error = e
            print(f'URLScan API request failed: {e}')
