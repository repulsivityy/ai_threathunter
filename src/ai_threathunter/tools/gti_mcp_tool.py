...
class GTIMCPTool(BaseTool):
    ...
    _debug_manager: Any = PrivateAttr()
    ...
    def _parse_ioc_response(self, text: str, ioc: str, ioc_type: str) -> IOCAnalysisResult:
        ...
        # Strategy: Create a result object where the description contains the full report.
    ...
    def _run(self, action: str, ioc: str, ioc_type: Optional[str] = None, **kwargs) -> Union[IOCAnalysisResult, BehavioralSummary, str]:
        ...
        try:
            if action == 'lookup_ioc':
                ...
                return self._parse_ioc_response(report_text, ioc, ioc_type)

            elif action == 'get_behaviour_summary':
                report_text = self.loop.run_until_complete(self._get_behaviour_summary(ioc))
                return self._parse_behavior_response(report_text, ioc)
            
            else:
                 return f"Error: Action {action} not fully supported in V2 Structured Mode yet."
